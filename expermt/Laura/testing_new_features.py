...
def adjust(len):
	n.side1.L = (len/(len/0.061619162))*elength(n.side1)
	n.dau1.L = (len/(len/0.061619162))*elength(n.dau1)
	n.dau2.L = (len/(len/0.061619162))*elength(n.dau2)
	n.side1.connect(n.main_shaft(0.6))
	n._normalize()
	n.side1.disconnect()
...
def test():
	...
	for k in lengths:
		...
		# M = m.Rin(m.side1(0))
		# N = n.Rin(n.side1(0))
		# adj_lst2.append(k/(k/0.061619162))
		# diff_lst.append(M-N)
		...
		print(f"{adj_lst[min_diff.index(min(min_diff))]} with a diff of {min(min_diff)} and the lengths are {n.side1.L/118.84, n.dau1.L/118.84, n.dau2.L/118.84}")
		...
		# 			diff_lst.append(M-N)
		# 			fin_results.append(k)
	return adj_lst2, diff_lst, fin_results, len_lst
...
def plot():
	...
	# plt.show()
	ax[0].plot(fin_results, len_lst, label="length")
