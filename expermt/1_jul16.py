    ...
    for i in range(maxsteps):
        ...
        if rec.proptest():
            ...
            if not hardrec.proptest():
                ...
                return (0-search.a)     # negative value to distinguish partial propogation results
            if rec.proptest() > 1:
                print("Double spike")
            if rec.recorded[0] > g_tstop - 6:   # change tstop if ap is taking too long to propogate (increased risk of false negative)
                g_tstop += 3    # addition because for some reason g_tstop does not like being multiplied
                print(g_tstop)
        if search.hi - search.lo <= acc:    # stop search when acc has been achieved
            break
    print(time.process_time() - ptstart)
    if i == maxsteps - 1:
        print("WARNING!!! U REACHED THE MAX STEPS!!")
    if abs(search.a - a) > 4*err: # if error is 4 times smaller than it should have been, create flag
...
def experiment(a, dists = None, lens = None, **kwargs):#,discon = True):
    ...
    h.define_shape()    # for consistent results when looking at movierun (theoretically)
...
def onesample(offset):
    ret = dtruthlite(offset = offset)
def simplegraph():
