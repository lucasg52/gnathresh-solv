...
class Rin_cell_v(BaseExpCell):
	def __init__(self, gid):
		...
		self.stim_b.diam = 0.2
		self.side1.diam = self.dau1.diam = 0.125992105
		self.stim_b.L = 4*118.84
		self.side1.L = 300
		self.dau1.L = 300
		self.dx = pow(2,-5)
		super().__init__(self.dx,3,gid)
	...
	def _setup_bioph(self):
		super()._setup_bioph()
		self.all = [self.stim_b, self.side1, self.dau1, self.main_shaft, self.IS, self.prop_site, self.soma]
		for sec in [self.stim_b, self.side1, self.dau1, self.main_shaft, self.IS, self.prop_site]:
			kinetics.insmod_Traub(sec, "axon")
		kinetics.insmod_Traub(self.soma, "soma")
	...
	def alt_Rin(self, loc):
		h.dt = 0.5
		side = self.side1.wholetree()
		for sec in [self.side1, self.dau1]:
			sec.gbar_nafTraub = 0
			sec.gbar_kdrTraub = 0
		stim = h.IClamp(loc)
		stim.amp = 200
